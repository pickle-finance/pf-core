...
describe("Testing documentation model", () => {
  test("Ensure all jars have documentation", async () => {
    const err: string[] = [];
    const assets = ALL_ASSETS.filter(
      (x) => x.enablement !== AssetEnablement.PERMANENTLY_DISABLED,
    );
    for (let i = 0; i < assets.length; i++) {
      const res: AssetDocumentationResult =
        DocsManager.getDocumentationForAssetId(
          assets[i].details.apiKey,
          "en",
          DocsFormat.HTML,
        );
      if (res === undefined) {
  ...
  test("Ensure all tokens have descriptions and documentation", async () => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      const desc = getTokenDescription(
        tokens[i].chain,
        tokens[i].id,
        language,
        format,
