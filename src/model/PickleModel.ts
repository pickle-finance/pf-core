...
export class PickleModel {
  ...
  defaultControllerForChain(chain: ChainNetwork) {
    if (chain === ChainNetwork.Ethereum) return ADDRESSES.Ethereum.controller;
    if (chain === ChainNetwork.Polygon) return ADDRESSES.Polygon.controller;
    if (chain === ChainNetwork.Arbitrum) return ADDRESSES.Arbitrum.controller;
    return undefined;
  }
  ...
  initializeChains(chains: Map<ChainNetwork, Provider | Signer>) {
    ...
    // ADD_CHAIN  TODO move into Chains if possible?
    setMulticallAddress(42161, "0x813715eF627B01f4931d8C6F8D2459F26E19137E");

    for (const key of allChains) {
  ...
  async loadHarvestData(jars: JarDefinition[], chain: ChainNetwork) {
    ...
    const balanceOfProm: Promise<BigNumber[]> = multicallProvider.all<
      BigNumber[]
    >(
    ...
    const availableProm: Promise<BigNumber[]> = multicallProvider2.all<
      BigNumber[]
    >(
      harvestableJars.map((oneJar) =>
  ...
  async ensureFarmsBalanceLoadedForProtocol(chain: ChainNetwork) {
    ...
    const protocolJarsWithFarmResults: string[] = await ethcallProvider.all<
      string[]
    >(
  ...
  async ensureExternalAssetBalanceLoaded() {
    ...
    for (let i = 0; i < external.length; i++) {
      ...
      if (behavior) {
        const bal = await behavior.getAssetHarvestData(
          external[i],
          this,
          null,
          null,
  ...
  async loadApyComponents() {
    ...
    const aprStats: AssetProjectedApr[] = await Promise.all(
      withBehaviors.map(async (x) => {
        return new JarBehaviorDiscovery()
          .findAssetBehavior(x)
