import { documentationAssetDefinitionToResult } from './docModel/documentationInterfaces';
...
function getDocs(language: string) {
  ...
  for (let i=0; i < ALL_JAR_DOCUMENTATION.length; i++) {
    ...
    docs.push(documentationAssetDefinitionToResult(language, ALL_JAR_DOCUMENTATION[i]));
  }
  return docs;
}
