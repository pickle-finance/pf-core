...
export async function getCurvePerformance(asset : JarDefinition, network = "mainnet") : Promise<PerformanceData> {
  ...
  export async function getYearnPerformance( asset: JarDefinition ) : Promise<PerformanceData> {
    ...
    const yearnRegistry = new ethers.Contract(
      ...
      yearnRegistryABI, provider
    );
    
    const vaultAddress = await yearnRegistry.latestVault(asset.depositToken.addr.toLowerCase(), 
        {gasLimit: 1000000});
  ...
  export async function getSwapPerformance(asset: JarDefinition) : Promise<PerformanceData> {
    ...
    for (let i = 0; i < pairDayResponse.length; i++) {
      ...
      if( i === 0 ) 
        returnObj.oneDay = totalApy / (i + 1);
      else if( i === 2 ) 
        returnObj.threeDay = totalApy / (i + 1);
      else if( i === 6 ) 
        returnObj.sevenDay = totalApy / (i + 1);
      else if( i === 29 ) 
        returnObj.thirtyDay = totalApy / (i + 1);
    }
    return returnObj;
  };
...
export async function getJarFarmPerformanceData(protocol: PerformanceData, 
    farmPerformance: number, data: AssetDatabaseEntry[]): Promise<JarFarmPerformanceData> {
  if ((!data )) {
    return undefined;
  }
...
export function getRatio(data, offset) : number {
    return  data.length >= offset ? data[data.length - (offset + 1)].ratio : undefined;
}
export function getBlock(data, offset) : number {
    return  data.length >= offset ? data[data.length - (offset + 1)].height : undefined;
}
...
export function getSamplePerformance(data: AssetDatabaseEntry[], offset:number) : number {
    // get current values
