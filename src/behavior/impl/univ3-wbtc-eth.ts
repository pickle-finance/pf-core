...
export class Uni3WbtcEth extends AbstractJarBehavior {
  async getDepositTokenPrice(
    ...
    const pJarUSD =
      model.priceOfSync("wbtc") * jarAmount0 +
  ...
  async getAssetHarvestData(
    ...
    const decimals0: number = _model.tokenDecimals("wbtc", definition.chain);
    const decimals1: number = _model.tokenDecimals("weth", definition.chain);

    const harvestableUSD =
      _model.priceOfSync("wbtc") *
