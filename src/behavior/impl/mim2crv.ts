...
export class Mim2Crv extends CurveJar {
  ...
  async getDepositTokenPrice(_definition: JarDefinition, model: PickleModel): Promise<number> {
  ...
  async getHarvestableUSD( jar: JarDefinition, model: PickleModel, _resolver: Signer | Provider): Promise<number> {
    const spell = new ethers.Contract(model.address("spell", ChainNetwork.Arbitrum), erc20Abi, model.providerFor(jar.chain));
    const strategy = new ethers.Contract(jar.details.strategyAddr, strategyAbi,model.providerFor(jar.chain));
    const [spellWallet, spellPrice, pending]: [BigNumber, number, BigNumber] = await Promise.all([
