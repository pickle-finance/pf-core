...
export class SteCrv extends AbstractJarBehavior {
  ...
  async getHarvestableUSD( jar: JarDefinition, model: PickleModel, resolver: Signer | Provider): Promise<number> {
    ...
    const strategy = new ethers.Contract(jar.details.strategyAddr, convexStrategyAbi, resolver);
    const [crvWallet, cvxWallet, ldoWallet, crvPrice, cvxPrice, ldoPrice, pending]: [
      BigNumber,
      BigNumber,
      BigNumber,
      number,
      ...
      lido.balanceOf(jar.details.strategyAddr).catch(() => BigNumber.from('0')),
    ...
    const harvestable = crvWallet
      ...
      .add(cvxWallet.add(pending[1]).mul(cvxPrice.toFixed()).add(ldoWallet.add(pending[2]).mul(ldoPrice.toFixed())));
