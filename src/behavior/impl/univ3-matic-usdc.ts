...
export class Uni3MaticUsdc extends AbstractJarBehavior {
  async getDepositTokenPrice(
    ...
    const pJarUSD =
      model.priceOfSync("matic") * jarAmount0 +
      model.priceOfSync("usdc") * jarAmount1;
  ...
  async getAssetHarvestData(
    ...
    const decimals0: number = _model.tokenDecimals("matic", definition.chain);
    const decimals1: number = _model.tokenDecimals("usdc", definition.chain);

    const harvestableUSD =
      _model.priceOfSync("matic") *
