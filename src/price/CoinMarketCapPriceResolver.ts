...
export class CoinMarketCapPriceResolver extends AbstractPriceResolver implements IPriceResolver{
    protected async fetchPricesBySearchId(searchNames: string[]): Promise<Map<string, number>> {
      ...
      const url2 = "https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest?CMC_PRO_API_KEY=2b734062-7f54-4850-883b-eea8eadbbd51&slug=" + joined;
      const res = await fetch(url).then((x) => x.json());
      let data = res.data;
      if (!data) {  // failsafe in case the first api key monthly limit is reached
