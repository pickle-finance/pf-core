...
export class ExternalTokenModel {
    ...
    constructor() {
        ...
        this.addToken(ChainNetwork.Ethereum, "pickle", "pickle-finance", "0x429881672B9AE42b8EbA0E26cD9C73711b891Ca5".toLowerCase());
    ...
    addToken(chain: ChainNetwork, id: string, cgid: string, addr: string, 
        dec = 18, style: ExternalTokenFetchStyle = ExternalTokenFetchStyle.BOTH) {
        this.chainTokens.get(chain).set(id, new ExternalToken(chain, id, cgid, addr, dec, style));
    }
    ...
    getAllTokensOutput() : IExternalToken[] {
        ...
        const chains = Chains.list();
        for( let i = 0; i < chains.length; i++ ) {
            const tmp : IExternalToken[] = this.getTokens(chains[i]).map((x)=>x.toOutputFormat());
            ret.push(...tmp);
        }
        return ret;
    }
