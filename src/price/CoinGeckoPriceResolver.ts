...
export const fetchCoingeckoPricesByContractNtimes = async (
  contractIds: string[], times: number
...
export const fetchCoinGeckoPricesBySearchId = async (geckoNames: string[]): Promise<Map<string,number>> => {
  const obj = {
    ids: geckoNames,
    vs_currencies: ["usd"],
  };
