...
import { NULL_ADDRESS } from "../model/PickleModel";
...
export async function getPosition(
  ...
  const info = uniV3Info[jar.depositToken.addr]
  ...
  const jarV3 = new ethers.Contract(jar.contract, jarAbi, provider);

  const [token0, token1, data, totalLiquidity, jarLiquidity] =
    await Promise.all([
      poolContract.token0(),
      poolContract.token1(),
      poolContract.slot0(),
