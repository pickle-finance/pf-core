...
import { JAR_ARBITRUM_MIM2CRV, JAR_MIM3CRV } from "../model/JarsAndFarms";
...
export async function calculateAbracadabraApyArbitrum(jar: JarDefinition, model: PickleModel) {
    ...
    const multicallSorbettiereFarm = new MulticallContract(
        SORBETTIERE_ARBITRUM, sorbettiereAbi);
    ...
    const [
      ...
      let tokenPrice: any;
      if (lpTokenAddress.toLowerCase() === JAR_ARBITRUM_MIM2CRV.depositToken.addr.toLowerCase()) {
