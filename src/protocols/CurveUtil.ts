...
export async function calculateCurveApyArbitrum(jar: JarDefinition, 
    model: PickleModel, swapAddress: string, _gauge: string, tokens: string[]) {
  ...
  const decimals: number[] = tokens.map((x)=>model.tokenDecimals(x, jar.chain));
  const prices: number[] = tokens.map((x)=>model.priceOfSync(x));
  let totalStakedUsd: number = 0;
  for( let i = 0; i < tokens.length; i++ ) {
    const oneEdec = "1" + Array.from(Array(decimals[i]).keys()).map((_x)=>"0").join(""); // this is retarded
