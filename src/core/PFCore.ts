...
export class PFCore {
  ...
  public static createPickleModelForSingleAsset(
    asset: string,
    chains: Map<ChainNetwork, Provider | Signer>,
  ): PickleModel | undefined {
    const onePickleAsset: PickleAsset = ALL_ASSETS.find(
      (x) =>
        x.details &&
        x.details.apiKey &&
        x.details.apiKey.toLowerCase() === asset.toLowerCase(),
    );
    if (onePickleAsset === undefined) return undefined;
  ...
  public static async createPickleModelAndUserModelForSingleAsset(
    ...
    wallet: string,
    chains: Map<ChainNetwork, Provider | Signer>,
  ): Promise<PickleAndUserModel | undefined> {
    const onePickleAsset: PickleAsset = ALL_ASSETS.find(
      (x) =>
        x.details &&
        x.details.apiKey &&
