...
export function minichefAddressForChain(network: ChainNetwork) {
  // TODO ADD_CHAIN
  if (network === ChainNetwork.Polygon) return ADDRESSES.Polygon.minichef;
  if (network === ChainNetwork.Arbitrum) return ADDRESSES.Arbitrum.minichef;
  return undefined;
}
